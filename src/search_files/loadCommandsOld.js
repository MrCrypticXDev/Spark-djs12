...
module.exports = async function(data, location) {
    ...
    data.dataStore.commandsOld = new DataStore();
    ...
    commands.forEach(i => {
        if (!data.dataStore.commandsOld.has(i.command.name.toLowerCase())) {
            data.dataStore.commandsOld.set(i.command.name.toLowerCase(), i)
        }
    })
    return commands;
}
